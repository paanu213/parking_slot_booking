...
import { CheckCircle2, XCircle, User, Ban, X, MoreVertical, Pencil, Plus, Clock, CalendarRange, CalendarCheck, Percent } from 'lucide-react';
...
const isActiveMonthly = (b: Booking) => {
...
const isRevenue = (b: Booking) => ['CONFIRMED', 'COMPLETED'].includes(b.status);
...
export const BookingsPage = () => {
  ...
  // ── Summary stats (over all bookings, not the active filter) ──
  const monthlyCount    = bookings.filter((b) => b.bookingType === 'MONTHLY').length;
  const hourlyCount     = bookings.length - monthlyCount;
  const activeMonthly   = bookings.filter(isActiveMonthly).length;
          ...
          <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-900/40 dark:bg-amber-900/10">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <SummaryCard
                ...
                value={formatINR(commissionData.totalCommission ?? 0)}
              />
              <SummaryCard
                icon={<CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-300" />}
                accent="bg-emerald-100 dark:bg-emerald-900/30"
                label="Paid"
                value={formatINR(commissionData.paidCommission ?? 0)}
              />
              <SummaryCard
                icon={<Clock className="h-4 w-4 text-red-600 dark:text-red-300" />}
                accent="bg-red-100 dark:bg-red-900/30"
                label="Pending"
                value={formatINR(commissionData.pendingCommission ?? 0)}
              />
            </div>

            {/* Per-space breakdown */}
            {Array.isArray(commissionData.spaces) && commissionData.spaces.length > 0 && (
              <div className="mt-4 overflow-x-auto rounded-lg border border-amber-100 bg-white dark:border-amber-900/30 dark:bg-slate-900">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {commissionData.spaces.map((s: any) => (
                      <tr key={s.id} className="border-t border-slate-100 dark:border-slate-800">
                        <td className="px-3 py-2 font-medium">{s.name}</td>
                        <td className="px-3 py-2 font-semibold">{formatINR(s.total)}</td>
                        <td className="px-3 py-2 text-emerald-600 dark:text-emerald-400">{formatINR(s.paid)}</td>
                        <td className={`px-3 py-2 ${s.pending > 0 ? 'font-semibold text-red-600 dark:text-red-400' : 'text-slate-400'}`}>
                          {formatINR(s.pending)}
                        </td>
                      </tr>
                    ))}
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={11} className="px-3 py-10 text-center text-sm text-slate-400">
                        No {planFilter.toLowerCase()} bookings.
                      </td>
                    </tr>
                  )}
                  {filtered.map((b) => {
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          {b.status === 'CANCELLED' ? (
                            ...
                            <>
                              <div className="font-medium">₹{Number(b.commissionAmount ?? 0).toLocaleString('en-IN')}</div>
