...
import {
  ...
  Clock, CalendarRange, Globe, User, MapPin, Building2,
} from 'lucide-react';
...
export const SlotBookingsPage = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['vendor-slot-bookings', id],
    queryFn: async () => (await api.get(`/vendor/slots/${id}/bookings`)).data,
    enabled: Boolean(id),
  });
                ...
                {sorted.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="border-t border-slate-100 dark:border-slate-800">
                      <td className="px-3 py-2 font-mono text-xs text-slate-500">{b.reference}</td>
                      ...
                      <td className="px-3 py-2">
                        ...
                        {customerSub && <div className="text-xs text-slate-400">{customerSub}</div>}
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-slate-500">{fmtDateTime(b.startAt)}</td>
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-slate-500">{fmtDateTime(b.endAt)}</td>
                      <td className="px-3 py-2 font-semibold">{formatINR(Number(b.totalAmount))}</td>
                      <td className="px-3 py-2">
