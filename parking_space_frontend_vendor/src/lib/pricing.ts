/**
 * Frontend pricing calculator — single source of truth for the vendor UI.
 *
 * KEEP IN SYNC with backend `src/lib/pricing.ts` and `@ps/types`. Same formula,
 * same behavior. We duplicate intentionally so the frontend doesn't need a
 * workspace build step in dev / CI.
 */
