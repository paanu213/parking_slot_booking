...
const LINKS: MenuLink[] = [
  { to: '/profile', label: 'Business profile', icon: Building2, description: 'Name, GST, payout UPI' },
  { to: '/profile', label: 'Account details', icon: UserCircle2, description: 'Personal info & contact' },
  { to: '/settings', label: 'Settings', icon: Settings, description: 'Notifications & preferences' },
  { to: '/security', label: 'Security', icon: ShieldCheck, description: 'Password & sessions' },
...
export const ProfileMenu = () => {
  ...
  const displayName = user?.fullName ?? 'Vendor';
          ...
          <div className="relative flex items-center gap-3 text-white">
            ...
            <div className="min-w-0 flex-1">
              ...
              <span className="mt-1 inline-flex items-center gap-1 rounded-full bg-white/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ring-1 ring-white/30">
                <Building2 className="h-3 w-3" /> Vendor workspace
              </span>
            </div>
          </div>
        </div>
